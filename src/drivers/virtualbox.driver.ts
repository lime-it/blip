...
export class VirtualboxDriver extends VmDriver {
  ...
  public async getMachineConfig(vmName: string): Promise<MachineConfig> {
    ...
    const diskSize = /\d+/.exec(res.stdout.split('\n').find(p => p.startsWith('Capacity')) || '')

    return {
      cpuCount: parseInt(tuples.find(p => p.startsWith('cpus='))?.replace('cpus=', '') || '1', 10),
      ramSizeMB: parseInt(tuples.find(p => p.startsWith('memory='))?.replace('memory=', '') || '1', 10),
      diskSizeMB: parseInt(diskSize ? diskSize[0] : '0', 10),
    }
  }
